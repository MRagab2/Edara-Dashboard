...
module.exports = class Product{
    ...
    async AddProduct(productInfo){
        ...
        const warehouseCheck = await query('SELECT * FROM warehouses WHERE id = ?', productInfo.warehouseID);
        if(!warehouseCheck[0]){
    ...
    async GetWarehouseProducts(warehouseID){
        const warehouseCheck = await query('SELECT * FROM warehouses WHERE id = ?',warehouseID); 
        if(!warehouseCheck[0]) 
            return {
                err: "Warehouse not found"
            };
